...
from shellguard.ssh import connection
from shellguard.ssh import keys as shellguardkeys
from shellguard.ssh import transport as shellTransport
from shellguard.ssh.userauth import HoneyPotSSHUserAuthServer
from shellguard.ssh_proxy import server_transport as proxyTransport
from shellguard.ssh_proxy.userauth import ProxySSHAuthServer
...
class ShellGuardSSHFactory(factory.SSHFactory):
    """
    ...
    ourVersionString: bytes = ShellGuardConfig.get(
    ...
    def __init__(self, backend, pool_handler):
        ...
        self.services = {
            ...
            b"ssh-connection": connection.ShellGuardSSHConnection,
        }
        super().__init__()
    ...
    def startFactory(self) -> None:
        ...
        try:
            public_key_auth = [
                i.encode("utf-8")
                for i in ShellGuardConfig.get("ssh", "public_key_auth").split(",")
            ]
        ...
        for key in public_key_auth:
            if key == b"ssh-rsa":
                rsaPubKeyString, rsaPrivKeyString = shellguardkeys.getRSAKeys()
                ...
                ecdsaPuKeyString, ecdsaPrivKeyString = shellguardkeys.getECDSAKeys()
                ...
                ed25519PubKeyString, ed25519PrivKeyString = shellguardkeys.geted25519Keys()
        ...
        self.ourVersionString = ShellGuardConfig.get(
            "ssh", "version", fallback="SSH-2.0-OpenSSH_6.0p1 Debian-4+deb7u2"
        ).encode("ascii")
    ...
    def buildProtocol(self, addr):
        ...
        @rtype: L{shellguard.ssh.transport.HoneyPotSSHTransport}
        ...
        try:
            t.supportedCiphers = [
                i.encode("utf-8") for i in ShellGuardConfig.get("ssh", "ciphers").split(",")
            ]
        except (NoOptionError, NoSectionError):
        ...
        try:
            t.supportedMACs = [
                i.encode("utf-8") for i in ShellGuardConfig.get("ssh", "macs").split(",")
            ]
        except (NoOptionError, NoSectionError):
        ...
        try:
            t.supportedCompressions = [
                i.encode("utf-8")
                for i in ShellGuardConfig.get("ssh", "compression").split(",")
            ]
        except (NoOptionError, NoSectionError):
