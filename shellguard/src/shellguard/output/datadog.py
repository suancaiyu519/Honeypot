...
class Output(shellguard.core.output.Output):
    def start(self) -> None:
        self.url = ShellGuardConfig.get("output_datadog", "url").encode("utf8")
        self.api_key = ShellGuardConfig.get(
            "output_datadog", "api_key", fallback=""
        ).encode("utf8")
        if len(self.api_key) == 0:
            log.msg("Datadog output module: API key is not defined.")
        self.ddsource = ShellGuardConfig.get(
            "output_datadog", "ddsource", fallback="shellguard"
        )
        self.ddtags = ShellGuardConfig.get("output_datadog", "ddtags", fallback="env:dev")
        self.service = ShellGuardConfig.get(
            "output_datadog", "service", fallback="honeypot"
        )
        self.hostname = ShellGuardConfig.get(
