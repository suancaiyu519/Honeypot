# -*- test-case-name: shellguard.test.protocol -*-
...
import shellguard.commands
from shellguard.core.config import ShellGuardConfig
from shellguard.shell import command, honeypot
from shellguard import data
...
class HoneyPotBaseProtocol(insults.TerminalProtocol, TimeoutMixin):
    ...
    for c in shellguard.commands.__all__:
        try:
            module = import_module(f"shellguard.commands.{c}")
    ...
    def connectionMade(self) -> None:
        ...
        log.msg(eventid="shellguard.session.params", arch=self.user.server.arch)

        idle_timeout = ShellGuardConfig.getint("honeypot", "idle_timeout", fallback=180)
        self.setTimeout(idle_timeout)

        # Source IP of client in user visible reports (can be fake or real)
        self.clientIP = ShellGuardConfig.get(
            "honeypot", "fake_addr", fallback=self.realClientIP
        )

        # Source IP of server in user visible reports (can be fake or real)
        if ShellGuardConfig.has_option("honeypot", "internet_facing_ip"):
            self.kippoIP = ShellGuardConfig.get("honeypot", "internet_facing_ip")
        else:
            try:
