...
# shellguard file system pickle files.
#
...
# such as the default: `src/shellguard/data/fs.pickle`.
#
# Donovan Hubbard
# Douglas Hubbard
# March 2013
...
from shellguard.shell.fs import FileNotFound
...
class fseditCmd(cmd.Cmd):
    def __init__(self, pickle_file_path):
        ...
        self.intro = (
            "\nShellGuard file system interactive editor\n" + "Type 'help' for help\n"
        )
    ...
    def help_about(self):
        print(
            "Kippo/ShellGuard stores information about its file systems in a "
            + "series of nested lists. Once the lists are made, they are "
            + "stored in a pickle file on the hard drive. Every time shellguard "
            + "gets a new client, it reads from the pickle file and loads "
            + "the fake file system into memory. By default this file "
            + "is /opt/shellguard/data/fs.pickle. Originally the script "
            + "/opt/shellguard/bin/createfs was used to copy the file system "
            ...
            + "shellguard process in order for the new file system to be "
