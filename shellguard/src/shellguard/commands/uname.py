...
def hardware_platform() -> str:
    return ShellGuardConfig.get("shell", "hardware_platform", fallback="x86_64")


def kernel_name() -> str:
    return ShellGuardConfig.get("shell", "kernel_name", fallback="Linux")


def kernel_version() -> str:
    return ShellGuardConfig.get("shell", "kernel_version", fallback="3.2.0-4-amd64")


def kernel_build_string() -> str:
    return ShellGuardConfig.get(
        "shell", "kernel_build_string", fallback="#1 SMP Debian 3.2.68-1+deb7u1"
    )


def operating_system() -> str:
    return ShellGuardConfig.get("shell", "operating_system", fallback="GNU/Linux")
