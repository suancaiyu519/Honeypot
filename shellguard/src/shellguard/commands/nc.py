...
from shellguard.core.network import communication_allowed, is_valid_port
...
nc_rate_limiter = RateLimiter(
    enabled=ShellGuardConfig.getboolean("honeypot", "nc_rate_limit_enabled", fallback=True),
    max_requests=ShellGuardConfig.getint("honeypot", "nc_rate_limit_requests", fallback=5),
    window_seconds=ShellGuardConfig.getint("honeypot", "nc_rate_limit_window", fallback=60),
    max_keys=ShellGuardConfig.getint("honeypot", "nc_rate_limit_max_hosts", fallback=1000)
)
