...
from shellguard.ssh_proxy.util import bin_string_to_hex, string_to_hex
...
class BackendSSHTransport(transport.SSHClientTransport, TimeoutMixin):
    """
    This class represents the transport layer from ShellGuard's proxy to the backend SSH server. It is responsible for
    ...
    def authenticateBackend(self, tried_username=None, tried_password=None):
        ...
        username = ShellGuardConfig.get("proxy", "backend_user")
        password = ShellGuardConfig.get("proxy", "backend_pass")
    ...
    def connectionLost(self, reason):
        if self.factory.server.pool_interface:
            log.msg(
                eventid="shellguard.proxy.client_disconnect",
