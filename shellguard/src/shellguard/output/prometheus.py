...
#  Metric objects
# ────────────────────────────────────────────
HOST_LABEL = ShellGuardConfig.get("honeypot", "hostname", fallback=socket.gethostname())
BUCKETS_LEN = (0, 4, 8, 12, 16, 20, 40)
BUCKETS_DUR = (1, 5, 15, 30, 60, 120, 300, 900, 1800, 3600)

sessions_total = Counter(
    "shellguard_sessions_total", "Total SSH/Telnet sessions", ["transport", "sensor"]
)
sessions_active = Gauge(
    "shellguard_sessions_active", "Active sessions", ["transport", "sensor"]
)
session_duration = Histogram(
    "shellguard_session_duration_seconds",
    "Session duration seconds",
    ["transport"],
    buckets=BUCKETS_DUR,
)
source_ip_total = Counter(
    "shellguard_source_ip_total", "Sessions per source IP", ["ip", "asn_country"]
)

source_ip_card = Gauge(
    "shellguard_source_ip_cardinality", "Unique source IPs seen", ["interval"]
)

login_attempts = Counter(
    "shellguard_login_attempts_total", "Login attempts", ["result", "username"]
)
password_length = Histogram(
    "shellguard_password_length", "Password length histogram", buckets=BUCKETS_LEN
)

commands_total = Counter(
    "shellguard_command_total", "Commands executed", ["command", "sensor"]
)

dl_bytes_total = Counter(
    "shellguard_file_download_bytes_total", "Downloaded bytes", ["protocol", "sensor"]
)
dl_time_hist = Histogram(
    "shellguard_file_download_time_seconds",
...
outbound_total = Counter(
    "shellguard_connection_outbound_total",
    "Outbound direct‑tcpip connections",
    ["dst_ip", "dst_port"],
)

loop_lag_hist = Histogram("shellguard_event_loop_lag_seconds", "Twisted reactor lag (s)")
py_exceptions = Counter(
    "shellguard_python_exceptions_total", "Uncaught Python exceptions", ["exception"]
)


class Output(shellguard.core.output.Output):
    def start(self) -> None:
        port = ShellGuardConfig.getint("output_prometheus", "port", fallback=9000)
    ...
    def write(self, event: dict) -> None:
        try:
            ...
            if eid == "shellguard.session.connect":
                self._on_session_connect(event)

            elif eid == "shellguard.session.closed":
                self._on_session_closed(event)

            elif eid in ("shellguard.login.success", "shellguard.login.failed"):
                self._on_login(event, success=eid.endswith("success"))

            elif eid == "shellguard.command.input":
                self._on_command(event)

            elif eid == "shellguard.session.file_download":
                self._on_download(event)

            elif eid == "shellguard.direct-tcpip.request":
