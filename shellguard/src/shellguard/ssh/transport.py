...
class HoneyPotSSHTransport(transport.SSHServerTransport, TimeoutMixin):
    ...
    auth_timeout: int = ShellGuardConfig.getint(
        "honeypot", "authentication_timeout", fallback=120
    )
    interactive_timeout: int = ShellGuardConfig.getint(
    ...
    def __repr__(self) -> str:
        ...
        @rtype: L{str}
        """
        return f"ShellGuard SSH Transport to {self.transport.getPeer().host}"
    ...
    def sendDisconnect(self, reason, desc):
        ...
        if b"bad packet length" not in desc:
            ...
            # this message is used to detect ShellGuard behaviour
