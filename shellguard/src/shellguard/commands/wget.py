...
from shellguard.core.artifact import Artifact
from shellguard.core.config import ShellGuardConfig
from shellguard.core.network import communication_allowed
from shellguard.core.rate_limiter import RateLimiter
from shellguard.shell.command import HoneyPotCommand

commands = {}

# Initialize rate limiter
wget_rate_limiter = RateLimiter(
    enabled=ShellGuardConfig.getboolean("honeypot", "wget_rate_limit_enabled", fallback=True),
    max_requests=ShellGuardConfig.getint("honeypot", "wget_rate_limit_requests", fallback=5),
    window_seconds=ShellGuardConfig.getint("honeypot", "wget_rate_limit_window", fallback=60),
    max_keys=ShellGuardConfig.getint("honeypot", "wget_rate_limit_max_hosts", fallback=1000)
)
...
class Command_wget(HoneyPotCommand):
    ...
    limit_size: int = ShellGuardConfig.getint("honeypot", "download_limit_size", fallback=0)
    quiet: bool = False
    ...
    def httpDownload(self, url: str) -> Any:
        ...
        # out_addr = None
        # if ShellGuardConfig.has_option("honeypot", "out_addr"):
        #     out_addr = (ShellGuardConfig.get("honeypot", "out_addr"), 0)
    ...
    def collectioncomplete(self, data: None) -> None:
        ...
        self.protocol.logDispatch(
            eventid="shellguard.session.file_download",
    ...
    def error(self, response):
        ...
        """
        self.protocol.logDispatch(
            eventid="shellguard.session.file_download.failed",
