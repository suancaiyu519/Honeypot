...
class Output(shellguard.core.output.Output):
    """
    MISP Upload Plugin for ShellGuard.
    ...
    def start(self) -> None:
        ...
        Start output plugin
        """
        misp_url = ShellGuardConfig.get("output_misp", "base_url")
        misp_key = ShellGuardConfig.get("output_misp", "api_key")
        misp_verifycert = ShellGuardConfig.getboolean("output_misp", "verify_cert")
        self.debug = ShellGuardConfig.getboolean("output_misp", "debug", fallback=False)
        self.publish = ShellGuardConfig.getboolean(
        ...
        self.handle_sessions = ShellGuardConfig.getboolean(
    ...
    def write(self, event: dict[str, Any]) -> None:
        ...
        if "session" in event and "src_ip" in event:
            ...
            elif event["eventid"] in ["shellguard.login.failed", "shellguard.login.success"]:
                success = event["eventid"] == "shellguard.login.success"
    ...
    def add_sighting(self, event, attribute):
        ...
        sighting.source = "{} (ShellGuard)".format(event["sensor"])
    ...
    def create_session_event(self, session_id, session_data):
        ...
        misp_event.info = f"{protocol} {title_prefix} from {session_data.get('src_ip', 'unknown')} (ShellGuard Session {session_id[:8]})"

        # Add standard tags
        self.add_standard_tags(misp_event)
        misp_event.add_tag("shellguard:session")
        ...
        session_object = MISPObject(name="shellguard-session", standalone=True)
        ...
        if session_data.get("login_attempts"):
            auth_object = MISPObject(name="shellguard-authentication", standalone=True)
        ...
        if session_data.get("commands"):
            command_object = MISPObject(name="shellguard-commands", standalone=True)
            ...
            for download in session_data["downloads"]:
                ...
                if "outfile" in download and download["shasum"] != "unknown":
                    ...
                    malware_attr.comment = (
                        f"File downloaded to ShellGuard honeypot in session {session_id}"
                    )
                    malware_attr.to_ids = True
                    misp_event.add_attribute(**malware_attr)

                # Still create the file object for structured data
                file_object = MISPObject(name="shellguard-file-download", standalone=True)
        ...
        if session_data.get("uploads"):
            for upload in session_data["uploads"]:
                file_object = MISPObject(name="shellguard-file-upload", standalone=True)
        ...
        summary_lines = [
            f"ShellGuard Honeypot Session Summary for {session_id}",
