...
class Output(shellguard.core.output.Output):
    ...
    def start(self):
        ...
        """
        host: str = ShellGuardConfig.get("output_redis", "host")
        port: int = ShellGuardConfig.getint("output_redis", "port")

        try:
            db = ShellGuardConfig.getint("output_redis", "db")
        except NoOptionError:
            db = 0

        try:
            password = ShellGuardConfig.get("output_redis", "password")
        except NoOptionError:
            password = None

        self.redis = redis.StrictRedis(host=host, port=port, db=db, password=password)

        self.keyname = ShellGuardConfig.get("output_redis", "keyname")

        try:
            self.send_method = SEND_METHODS[
                ShellGuardConfig.get("output_redis", "send_method")
            ]
