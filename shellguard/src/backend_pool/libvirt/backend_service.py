...
class LibvirtBackendService:
    def __init__(self) -> None:
        ...
        libvirt_uri: str = ShellGuardConfig.get(
        ...
        if self.conn is None:
            log.msg(
                eventid="shellguard.backend_pool.libvirtd",
    ...
    def create_guest(
        ...
        if dom is None:
            log.msg(
                eventid="shellguard.backend_pool.libvirtd", format="Failed to create guest"
            )
            return None
    ...
    def destroy_guest(self, domain: Any, snapshot: str) -> None:
        ...
        try:
            ...
            #   - no snapshot dir was defined (using shellguard's root dir) - should not happen but prevent it
            if (
                (
                    not ShellGuardConfig.getboolean(
                        "backend_pool", "save_snapshots", fallback=True
                    )
                    or ShellGuardConfig.get("backend_pool", "snapshot_path", fallback=None)
                    is None
                )
    ...
    def __destroy_all_guests(self) -> None:
        ...
        for domain_id in domains:
            ...
            if d.name().startswith("shellguard"):
                try:
                    d.destroy()
    ...
    def __destroy_all_networks(self) -> None:
        ...
        for network in networks:
            if network.startswith("shellguard"):
    ...
    def __destroy_all_network_filters(self) -> None:
        ...
        for nw_filter in network_filters:
            if nw_filter.startswith("shellguard"):
                n = self.conn.nwfilterLookupByName(nw_filter)
                n.undefine()

    def destroy_all_shellguard(self) -> None:
