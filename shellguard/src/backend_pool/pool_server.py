...
class PoolServer(Protocol):
    ...
    def __init__(self, factory: PoolServerFactory) -> None:
        ...
        if self.use_nat:
            self.nat_public_ip: str = ShellGuardConfig.get("backend_pool", "nat_public_ip")
    ...
    def dataReceived(self, data: bytes) -> None:
        ...
        if res_op == RES_OP_I:
            ...
            log.msg(
                eventid="shellguard.backend_pool.server",
            ...
            try:
                ...
                ssh_port: int = ShellGuardConfig.getint(
                    "backend_pool", "guest_ssh_port", fallback=22
                )
                telnet_port: int = ShellGuardConfig.getint(
...
class PoolServerFactory(Factory):
    ...
    def stopFactory(self) -> None:
        log.msg(eventid="shellguard.backend_pool.server", format="Stopping backend pool...")
        if self.pool_service:
