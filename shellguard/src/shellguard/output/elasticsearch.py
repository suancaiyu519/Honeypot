...
class Output(shellguard.core.output.Output):
    ...
    def start(self):
        host = ShellGuardConfig.get("output_elasticsearch", "host")
        port = ShellGuardConfig.get("output_elasticsearch", "port")
        self.index = ShellGuardConfig.get("output_elasticsearch", "index")
        self.type = ShellGuardConfig.get("output_elasticsearch", "type", fallback="_doc")
        self.pipeline = ShellGuardConfig.get("output_elasticsearch", "pipeline")
        # new options (creds + https)
        username = ShellGuardConfig.get("output_elasticsearch", "username", fallback=None)
        password = ShellGuardConfig.get("output_elasticsearch", "password", fallback=None)
        use_ssl = ShellGuardConfig.getboolean("output_elasticsearch", "ssl", fallback=False)
        ca_certs = ShellGuardConfig.get("output_elasticsearch", "ca_certs", fallback=None)
        verify_certs = ShellGuardConfig.getboolean(
