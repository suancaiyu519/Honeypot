...
class PoolClient(Protocol):
    ...
    def send_initialisation(self):
        ...
        """
        max_vms = ShellGuardConfig.getint("proxy", "pool_max_vms", fallback=2)
        vm_unused_timeout = ShellGuardConfig.getint(
            "proxy", "pool_vm_unused_timeout", fallback=600
        )
        share_guests = ShellGuardConfig.getboolean(
    ...
    def dataReceived(self, data):
        ...
        if res_op == b"i":
            ...
            if res_code != 0:
                log.msg(
                    eventid="shellguard.pool_client",
