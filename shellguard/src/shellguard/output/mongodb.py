...
class Output(shellguard.core.output.Output):
    ...
    def start(self):
        db_addr = ShellGuardConfig.get("output_mongodb", "connection_string")
        db_name = ShellGuardConfig.get("output_mongodb", "database")
    ...
    def write(self, event):
        ...
        if eventid == "shellguard.session.connect":
        ...
        elif eventid in ["shellguard.command.input", "shellguard.command.failed"]:
        ...
        elif eventid == "shellguard.log.closed":
        ...
        elif eventid == "shellguard.direct-tcpip.request":
            self.insert_one(self.col_ipforwards, event)

        elif eventid == "shellguard.direct-tcpip.data":
