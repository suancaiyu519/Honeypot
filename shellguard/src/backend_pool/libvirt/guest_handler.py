...
def create_guest(connection: Any, mac_address: str, guest_unique_id: str) -> tuple[Any, str]:
    ...
    configuration_file: str = os.path.join(
        ShellGuardConfig.get(
            "backend_pool", "config_files_path", fallback="src/shellguard/data/pool_configs"
        ),
        ShellGuardConfig.get("backend_pool", "guest_config", fallback="default_guest.xml"),
    )

    version_tag: str = ShellGuardConfig.get("backend_pool", "guest_tag", fallback="guest")
    base_image: str = ShellGuardConfig.get("backend_pool", "guest_image_path")
    hypervisor: str = ShellGuardConfig.get(
        "backend_pool", "guest_hypervisor", fallback="qemu"
    )
    memory: int = ShellGuardConfig.getint("backend_pool", "guest_memory", fallback=128)
    qemu_machine: str = ShellGuardConfig.get(
    ...
    if not os.path.isfile(base_image):
        log.msg(
            eventid="shellguard.backend_pool.guest_handler",
    ...
    kernel_image: str = ShellGuardConfig.get(
    ...
    try:
        ...
        snapshot_path: str = backend_pool.util.to_absolute_path(
            ShellGuardConfig.get("backend_pool", "snapshot_path")
        )
    ...
    guest_config = guest_xml.format(
        guest_name="shellguard-" + version_tag + "_" + guest_unique_id,
        disk_image=disk_img,
        base_image=base_image,
