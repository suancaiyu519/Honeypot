...
"""
This module contains code to deal with ShellGuard's configuration
"""
...
class EnvironmentConfigParser(configparser.ConfigParser):
    ...
    def has_option(  # type: ignore[override,unused-ignore]
        ...
        if to_environ_key("_".join(("shellguard", section, option))) in environ:
            return True
        return super().has_option(section, option)

    def get(self, section: str, option: str, *, raw: bool = False, **kwargs) -> str:  # type: ignore
        key: str = to_environ_key("_".join(("shellguard", section, option)))
        if key in environ:
            return environ[key]
...
def get_config_path() -> list[str]:
    ...
    config_files = [
        join(root, "etc/shellguard.cfg.dist"),
        "/etc/shellguard/shellguard.cfg",
        join(root, "etc/shellguard.cfg"),
        join(root, "shellguard.cfg"),
    ]
...
ShellGuardConfig = readConfigFile(get_config_path())
