...
class Output(shellguard.core.output.Output):
    """
    RabbitMQ output plugin for ShellGuard using event types as routing keys,
    ...
    def start(self):
        ...
        self.host = ShellGuardConfig.get("output_rmq", "host", fallback="localhost")
        self.port = ShellGuardConfig.getint("output_rmq", "port", fallback=5672)
        self.username = ShellGuardConfig.get("output_rmq", "username", fallback="guest")
        self.password = ShellGuardConfig.get("output_rmq", "password", fallback="guest")
        self.vhost = ShellGuardConfig.get("output_rmq", "vhost", fallback="/")
        self.exchange = ShellGuardConfig.get("output_rmq", "exchange", fallback="shellguard")
        self.exchange_type = ShellGuardConfig.get(
    ...
    def write(self, event):
        ...
        routing_key = event.get("eventid", "shellguard.unknown")
