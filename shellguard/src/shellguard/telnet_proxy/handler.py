...
from shellguard.core import ttylog
from shellguard.core.checkers import HoneypotPasswordChecker
...
class TelnetHandler:
    def __init__(self, server):
        ...
        self.spoofAuthenticationData = ShellGuardConfig.getboolean(
            "proxy", "telnet_spoof_authentication", fallback=True
        )

        self.backendLogin = ShellGuardConfig.get("proxy", "backend_user").encode()
        self.backendPassword = ShellGuardConfig.get("proxy", "backend_pass").encode()

        self.usernameInNegotiationRegex = ShellGuardConfig.get(
            "proxy", "telnet_username_in_negotiation_regex", raw=True
        ).encode()
        self.usernamePromptRegex = ShellGuardConfig.get(
            "proxy", "telnet_username_prompt_regex", raw=True
        ).encode()
        self.passwordPromptRegex = ShellGuardConfig.get(
        ...
        self.ttylogPath = ShellGuardConfig.get("honeypot", "ttylog_path", fallback=".")
        self.ttylogEnabled = ShellGuardConfig.getboolean(
    ...
    def sendBackend(self, data: bytes) -> None:
        ...
        for packet in self.backend_buffer:
            ...
            if ShellGuardConfig.getboolean("proxy", "log_raw", fallback=False):
        ...
        if parent == "frontend" and self.authDone:
            ...
            if b"\r" in data:
                if len(self.currentCommand) > 0:
                    log.msg(
                        eventid="shellguard.command.input",
        ...
        if b"\r" in self.currentData:
            ...
            if HoneypotPasswordChecker().checkUserPass(
                ...
                self.server.setTimeout(
                    ShellGuardConfig.getint("honeypot", "idle_timeout", fallback=300)
                )
            else:
