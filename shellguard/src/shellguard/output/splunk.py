...
class Output(shellguard.core.output.Output):
    ...
    def start(self) -> None:
        self.token = ShellGuardConfig.get("output_splunk", "token")
        self.url = ShellGuardConfig.get("output_splunk", "url").encode("utf8")
        self.index = ShellGuardConfig.get("output_splunk", "index", fallback="main")
        self.source = ShellGuardConfig.get("output_splunk", "source", fallback="shellguard")
        self.sourcetype = ShellGuardConfig.get(
            "output_splunk", "sourcetype", fallback="shellguard"
        )
        self.host = ShellGuardConfig.get("output_splunk", "host", fallback=None)
    ...
    def postentry(self, entry):
        ...
        headers = http_headers.Headers(
            {
                b"User-Agent": [b"ShellGuard SSH Honeypot"],
