# -*- test-case-name: shellguard.test.utils -*-
...
class ShellGuardDailyLogFile(logfile.DailyLogFile):
    """
...
def logger():
    ...
    """
    directory = ShellGuardConfig.get("honeypot", "log_path", fallback=".")

    logtype = ShellGuardConfig.get("honeypot", "logtype", fallback="plain")
    if logtype == "rotating":
        shellguardlog = ShellGuardDailyLogFile("shellguard.log", directory)
    elif logtype == "plain":
        shellguardlog = open(Path(directory, "shellguard.log"), "a", encoding="utf-8")
    else:
        raise ValueError
    ...
    return textFileLogObserver(shellguardlog, timeFormat=timeFormat)
