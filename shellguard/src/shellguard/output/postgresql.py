...
class Output(shellguard.core.output.Output):
    """
    PostgreSQL output for ShellGuard
    """
    ...
    def start(self):
        ...
        port = ShellGuardConfig.getint("output_postgresql", "port", fallback=5432)

        try:
            self.db = ReconnectingPostgreSQLConnectionPool(
                "psycopg2",
                host=ShellGuardConfig.get("output_postgresql", "host"),
                database=ShellGuardConfig.get("output_postgresql", "database"),
                user=ShellGuardConfig.get("output_postgresql", "username"),
                password=ShellGuardConfig.get("output_postgresql", "password", raw=True),
                port=port,
                cp_min=1,
                cp_max=1,
            )
