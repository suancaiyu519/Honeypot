...
class ShellGuardSSHChannel(channel.SSHChannel):
    """
    ...
    bytesReceived: int = 0
    bytesWritten: int = 0
    name: bytes = b"shellguard-ssh-channel"
    startTime: float = 0.0
    ttylogPath: str = ShellGuardConfig.get("honeypot", "log_path", fallback=".")
    downloadPath: str = ShellGuardConfig.get("honeypot", "download_path", fallback=".")
    ttylogEnabled: bool = ShellGuardConfig.getboolean("honeypot", "ttylog", fallback=True)
    bytesReceivedLimit: int = ShellGuardConfig.getint(
    ...
    def __repr__(self) -> str:
        ...
        @rtype: L{str}
        """
        return f"ShellGuard SSH Channel {self.name.decode()}"
