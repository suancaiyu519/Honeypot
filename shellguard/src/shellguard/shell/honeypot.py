...
from shellguard.shell import fs
from shellguard.shell.parser import CommandParser
from shellguard.shell.pipe import PipeProtocol
...
class HoneyPotShell:
    ...
    def lineReceived(self, line: str) -> None:
        log.msg(eventid="shellguard.command.input", input=line, format="CMD: %(input)s")
    ...
    def runCommand(self):
        ...
        for index, cmd in reversed(list(enumerate(cmd_array))):
            ...
            if cmdclass:
                ...
                log.msg(
                    eventid="shellguard.command.failed",
                    input=cmd["command"] + " " + " ".join(cmd["rargs"]),
    ...
    def showPrompt(self) -> None:
        ...
        prompt = ""
        if ShellGuardConfig.has_option("honeypot", "prompt"):
            prompt = ShellGuardConfig.get("honeypot", "prompt")
            prompt += " "
        else:
