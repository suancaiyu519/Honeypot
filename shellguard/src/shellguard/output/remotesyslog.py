...
class Output(shellguard.core.output.Output):
    def start(self):
        self.host = ShellGuardConfig.get(
            "output_remotesyslog", "host", fallback="127.0.0.1"
        )

        self.port = int(ShellGuardConfig.get("output_remotesyslog", "port", fallback="514"))

        protocol = ShellGuardConfig.get(
            "output_remotesyslog", "protocol", fallback="udp"
        ).lower()

        self.logger = logging.getLogger("shellguardLogger")
