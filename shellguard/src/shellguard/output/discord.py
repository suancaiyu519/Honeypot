...
class Output(shellguard.core.output.Output):
    ...
    # Lifecycle
    def start(self):
        self.url = ShellGuardConfig.get("output_discord", "url").encode("utf8")
        ...
        self._default_delay: float = ShellGuardConfig.getfloat("output_discord", "default_delay", fallback=DEFAULT_DELAY)
        self._retry_delay: float = ShellGuardConfig.getfloat("output_discord", "retry_delay", fallback=RETRY_DELAY)
        self._max_retries: int = ShellGuardConfig.getint("output_discord", "max_retries", fallback=MAX_RETRIES)
    ...
    def _build_embed(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        eventid = str(event.get("eventid", "ShellGuard Event"))
