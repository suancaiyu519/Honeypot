...
def create_filter(connection: Any) -> Any:
    ...
    try:
        ...
        log.err(
            eventid="shellguard.backend_pool.network_handler",
            format="Filter already exists: %(error)s",
            error=e,
        )
        return connection.nwfilterLookupByName("shellguard-default-filter")
...
def create_network(connection: Any, network_table: dict[str, str]) -> Any:
    ...
    try:
        ...
        return connection.networkLookupByName("shellguard")
