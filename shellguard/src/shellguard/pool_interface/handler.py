...
from shellguard.pool_interface.client import PoolClientFactory
...
class PoolHandler:
    ...
    def __init__(self, pool_host, pool_port, shellguard_plugin):
        # used for initialisation only
        self.shellguard_plugin = shellguard_plugin
    ...
    def initial_pool_connection_success(self, client):
        log.msg("Initialising pool with ShellGuard settings...")
    ...
    def initialisation_response(self, res_code):
        ...
        if res_code == 0:
            ...
            self.shellguard_plugin.pool_ready()
        else:
