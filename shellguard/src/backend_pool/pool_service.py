...
class PoolService:
    ...
    def __init__(self, nat_service):
        ...
        # file configs
        self.ssh_port: int = ShellGuardConfig.getint(
            "backend_pool", "guest_ssh_port", fallback=-1
        )
        self.telnet_port: int = ShellGuardConfig.getint(
        ...
        self.use_nat: bool = ShellGuardConfig.getboolean(
            "backend_pool", "use_nat", fallback=True
        )

        if self.ssh_port == -1 and self.telnet_port == -1:
            log.msg(
                eventid="shellguard.backend_pool.service",
    ...
    def start_pool(self) -> None:
        ...
        self.qemu.destroy_all_shellguard()
        ...
        recycle_period = ShellGuardConfig.getint(
    ...
    def stop_pool(self) -> None:
        ...
        log.msg(eventid="shellguard.backend_pool.service", format="Trying pool clean stop")
        ...
        if (not self.local_pool and self.use_nat) or self.pool_only:
            log.msg(
                eventid="shellguard.backend_pool.service", format="Free all NAT bindings"
            )
