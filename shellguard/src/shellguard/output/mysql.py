...
class Output(shellguard.core.output.Output):
    ...
    def start(self):
        self.debug = ShellGuardConfig.getboolean("output_mysql", "debug", fallback=False)
        port = ShellGuardConfig.getint("output_mysql", "port", fallback=3306)
        try:
            self.db = ReconnectingConnectionPool(
                "mysql.connector",
                host=ShellGuardConfig.get("output_mysql", "host"),
                db=ShellGuardConfig.get("output_mysql", "database"),
                user=ShellGuardConfig.get("output_mysql", "username"),
                passwd=ShellGuardConfig.get("output_mysql", "password", raw=True),
                port=port,
                cp_min=1,
                cp_max=1,
                charset="utf8mb4",
    ...
    def write(self, event):
        ...
        elif event["eventid"] == "shellguard.session.input":
            self.simpleQuery(
