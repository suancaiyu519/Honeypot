...
class Output(shellguard.core.output.Output):
    ...
    def start(self):
        server = ShellGuardConfig.get("output_xmpp", "server")
        user = ShellGuardConfig.get("output_xmpp", "user")
        password = ShellGuardConfig.get("output_xmpp", "password")
        muc = ShellGuardConfig.get("output_xmpp", "muc")
    ...
    def run(self, application, jidstr, password, muc, server):
        ...
        if ShellGuardConfig.getboolean("output_xmpp", "debug", fallback=False):
