...
class Output(shellguard.core.output.Output):
    ...
    def start(self):
        self.auth_key = ShellGuardConfig.get("output_dshield", "auth_key")
        self.userid = ShellGuardConfig.get("output_dshield", "userid")
        self.batch_size = ShellGuardConfig.getint("output_dshield", "batch_size")
        self.debug = ShellGuardConfig.getboolean("output_dshield", "debug", fallback=False)
    ...
    def write(self, event):
        if (
            event["eventid"] == "shellguard.login.success"
            or event["eventid"] == "shellguard.login.failed"
        ):
