...
class Output(shellguard.core.output.Output):
    ...
    def generate_random_log_id(self):
        ...
        return f"shellguardlog-{random_log_id}"

    def sendLogs(self, event):
        log_id = self.generate_random_log_id()
        # Initialize service client with default config file
        current_time = datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None)
        self.log_ocid = ShellGuardConfig.get("output_oraclecloud", "log_ocid")
        self.hostname = ShellGuardConfig.get("honeypot", "hostname")
    ...
    def start(self):
        ...
        """
        authtype = ShellGuardConfig.get("output_oraclecloud", "authtype")
        ...
        if authtype == "instance_principals":
            ...
            tenancy_ocid = ShellGuardConfig.get("output_oraclecloud", "tenancy_ocid")
            user_ocid = ShellGuardConfig.get("output_oraclecloud", "user_ocid")
            region = ShellGuardConfig.get("output_oraclecloud", "region")
            fingerprint = ShellGuardConfig.get("output_oraclecloud", "fingerprint")
            keyfile = ShellGuardConfig.get("output_oraclecloud", "keyfile")
