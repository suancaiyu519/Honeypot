...
from shellguard.core.fingerprint import (
    generate_ja4,
    generate_ja4h,
...
def shellguardOpenConnectForwardingClient(remoteWindow, remoteMaxPacket, data, avatar):
    """
    ...
    log.msg(
        eventid="shellguard.direct-tcpip.request",
    ...
    # Forward redirect
    redirectEnabled: bool = ShellGuardConfig.getboolean(
        "ssh", "forward_redirect", fallback=False
    )
    if redirectEnabled:
        redirects = {}
        items = ShellGuardConfig.items("ssh")
        for i in items:
        ...
        if remoteHP[1] in redirects:
            ...
            log.msg(
                eventid="shellguard.direct-tcpip.redirect",
    ...
    # TCP tunnel
    tunnelEnabled: bool = ShellGuardConfig.getboolean(
    ...
    if tunnelEnabled:
        ...
        if remoteHP[1] in tunnels:
            ...
            log.msg(
                eventid="shellguard.direct-tcpip.tunnel",
...
class SSHConnectForwardingChannel(forwarding.SSHConnectForwardingChannel):
    ...
    name = b"shellguard-forwarded-direct-tcpip"
...
class FakeForwardingChannel(forwarding.SSHConnectForwardingChannel):
    ...
    name = b"shellguard-discarded-direct-tcpip"
    ...
    def dataReceived(self, data: bytes) -> None:
        ...
        if len(data) >= 6 and data[0] == 0x16 and data[5] == 0x01:
            ...
            if tls_info:
                try:
                    ...
                    log.msg(
                        eventid="shellguard.direct-tcpip.ja4",
        ...
        # Check for HTTP request
            ...
            if http_info:
                try:
                    ...
                    log.msg(
                        eventid="shellguard.direct-tcpip.ja4h",
        ...
        log.msg(
            eventid="shellguard.direct-tcpip.data",
...
class TCPTunnelForwardingChannel(forwarding.SSHConnectForwardingChannel):
    ...
    name = b"shellguard-tunneled-direct-tcpip"
    ...
    def dataReceived(self, data: bytes) -> None:
        ...
        log.msg(
            eventid="shellguard.tunnelproxy-tcpip.data",
