#!/bin/bash
# ShellGuard Docker 管理脚本
# 用途: 统一管理 ShellGuard 蜜罐容器的部署、启动、停止、日志等操作
...
SHELLGUARD_DIR="${SCRIPT_DIR}"
DOCKER_DIR="${SHELLGUARD_DIR}/docker"

# 显示使用帮助
show_help() {
    echo -e "${BLUE}ShellGuard Docker 管理脚本${NC}\n"
    echo "用法: $0 <命令> [选项]"
    echo ""
    echo "可用命令:"
    echo -e "  ${GREEN}deploy${NC}     部署并启动 ShellGuard"
    echo -e "  ${GREEN}start${NC}      启动 ShellGuard 容器"
    echo -e "  ${GREEN}stop${NC}       停止 ShellGuard 容器"
    echo -e "  ${GREEN}restart${NC}    重启 ShellGuard 容器"
...
check_directory() {
    if [ ! -d "${DOCKER_DIR}" ]; then
        ...
        echo -e "${YELLOW}提示: 请确保已克隆 ShellGuard 仓库到 ${SHELLGUARD_DIR}${NC}"
        exit 1
    fi
}
...
deploy() {
    ...
    echo -e "${YELLOW}→${NC} 开始构建和启动 ShellGuard 容器..."
...
# 启动函数
start() {
    echo -e "${YELLOW}→${NC} 启动 ShellGuard 容器..."
    check_directory
    cd "${DOCKER_DIR}"
    docker compose start
    echo -e "${GREEN}✓${NC} ShellGuard 已启动"
    docker compose ps
}

# 停止函数
stop() {
    echo -e "${YELLOW}→${NC} 停止 ShellGuard 容器..."
    check_directory
    cd "${DOCKER_DIR}"
    docker compose stop
    echo -e "${GREEN}✓${NC} ShellGuard 已停止"
    docker compose ps
}

# 重启函数
restart() {
    echo -e "${YELLOW}→${NC} 重启 ShellGuard 容器..."
    check_directory
    cd "${DOCKER_DIR}"
    docker compose restart
    echo -e "${GREEN}✓${NC} ShellGuard 已重启"
    docker compose ps
}

# 查看状态
status() {
    echo -e "${BLUE}=== ShellGuard 容器状态 ===${NC}"
    check_directory
    cd "${DOCKER_DIR}"
    docker compose ps
...
# 查看实时日志
logs() {
    echo -e "${YELLOW}→${NC} 实时查看 ShellGuard 日志 (按 Ctrl+C 退出)..."
    check_directory
    cd "${DOCKER_DIR}"
...
logs_tail() {
    ...
    check_directory
    cd "${DOCKER_DIR}"
    docker compose logs --tail=${lines} shellguard
}
...
jsons() {
    ...
    # 获取 volume 挂载路径
    VOLUME_PATH=$(docker volume inspect docker_shellguard-var --format '{{ .Mountpoint }}' 2>/dev/null)
    ...
    LOG_FILE="$VOLUME_PATH/log/shellguard/shellguard_chinese.json"
...
# 停止并删除容器
down() {
    echo -e "${YELLOW}→${NC} 停止并删除 ShellGuard 容器（保留数据卷）..."
    check_directory
    cd "${DOCKER_DIR}"
    docker compose down
...
down_all() {
    ...
    if [ "$confirm" = "yes" ]; then
        echo -e "${YELLOW}→${NC} 停止并删除 ShellGuard 容器和数据卷..."
        check_directory
        cd "${DOCKER_DIR}"
...
# 进入容器 shell
shell() {
    echo -e "${YELLOW}→${NC} 进入 ShellGuard 容器 shell..."
    check_directory
    cd "${DOCKER_DIR}"
    docker compose exec shellguard sh
}
...
add_user() {
    ...
    # 检查容器是否运行
    CONTAINER_ID=$(docker compose ps -q shellguard)
    ...
    # 获取 volume 挂载路径
    VOLUME_PATH=$(docker volume inspect docker_shellguard-etc --format '{{ .Mountpoint }}' 2>/dev/null)
